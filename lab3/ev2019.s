...
//Se apagan todos los segmentos y digitos
        ...
        MOV     R0,#DIG_MASK
        STR     R0,[R1,#DIG_OFFSET]
//Se apaga el segmento punto sin modificar los restantes dispositivos en el GPIO 5
        LDR     R0,=SEG_DP_MASK
        LDR     R2,[R1,#SEG_DP_OFFSET]
        ORR     R0,R2
        STR     R0,[R1,#SEG_DP_OFFSET]
...
//Si se prende el display 2 que se prenda tambien el punto
        CMP     R3,#1
        ITTTT     EQ
        LDREQ   R0,=SEG_DP_MASK
        LDREQ   R2,[R1,#SEG_DP_OFFSET]
        ORREQ   R0,R2
        STREQ   R0,[R1,#SEG_DP_OFFSET]
//Incrementamos el digito a refrescar y si llega a 3 lo reiniciamos a 0
        ADD     R3,#1
        CMP     R3,#3
        IT      EQ
        MOVEQ   R3,#0
//Veo si el cartel llego a 0 y prendo el LED
        LDR     R0,=Segundos
        LDRB    R1,[R0],#1
        LDRB    R2,[R0],#1
        ADD     R1,R2
