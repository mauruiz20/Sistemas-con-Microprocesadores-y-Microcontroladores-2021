...
// Valores inicales de las variables
        ...
        LDR     R0,=Velocidad
        MOV     R1,#10                  //Como el Systick interrumpe cada 1 ms, quiero que
        STRH    R1,[R0]                 //cuente cada 1 cencecima de segundo 1/100
        MOV     R7,#0                   //R7 = 0 indica que el jugador 1 no paro la cuenta
        MOV     R8,#0                   //R8 = 0 indica que el jugador 2 no paro la cuenta
// Define los punteros para usar en el programa
        LDR     R4,=GPIO_PIN0
refrescar:  
        LDR     R3,[R4,#BTN_OFFSET]
...
//Incrementamos el refresco y si llega a 4 (cuatro displays) lo reiniciamos a 0
        ADD     R5,#1
        CMP     R5,#4
        IT      EQ
        MOVEQ   R5,#0
...
retardo:
        ...
        B       retardo
jugador1:
        CMP     R7,#0           //Si apreta el jugador 1 se congela la cuenta
        ITE     EQ              //Si apreta de nuevo continua la cuenta        
        MOVEQ   R7,#1
        MOVNE   R7,#0
        LDR     R0,=#0x300000
        B       retardo
jugador2:
        CMP     R8,#0           //Si apreta el jugador 2 se congela la cuenta
        ITE     EQ              //Si apreta de nuevo continua la cuenta 
        MOVEQ   R8,#1
        MOVNE   R8,#0
        LDR     R0,=#0x300000
...
acelerar:
        LDR     R1,=Velocidad
        LDRH    R0,[R1]
        ADD     R0,R0,#200               //Sumo 20 centesimos de velocidad
        STRH    R0,[R1]
        LDR     R0,=#0x100000
        B       retardo
desacelerar:
        LDR     R1,=Velocidad
        LDRH    R0,[R1]
        SUB     R0,R0,#200               //Resto 20 centesimos de velocidad
        STRH    R0,[R1]
        LDR     R0,=#0x100000
        B       retardo
...
actualiza_reloj:
        ...
        PUSH    {LR}
        CMP     R7,#0           //Si el jugador 1 para la cuenta se congela su cuenta
        ITE     EQ
        MOVEQ   R0,#1   
        MOVNE   R0,#0
        LDRB    R1,[R3]
        ...
        CMP     R8,#0           //Si el jugador 2 para la cuenta se congela su cuenta
        ITE     EQ
        MOVEQ   R0,#1
        MOVNE   R0,#0
        LDRB    R1,[R3]
...
actualiza_digitos:
        ...
        CMP     R7,#0           //Si R7 es distinto de 0 el jugador 1 gano y se congela la cuenta
        IT      EQ
        STRBEQ  R4,[R0]
        ...
        CMP     R8,#0           //Si R8 es distinto de 0 el jugador 2 gano y se congela la cuenta
        IT      EQ
        STRBEQ  R4,[R0,#2]
