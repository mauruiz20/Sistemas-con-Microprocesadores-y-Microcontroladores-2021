...
reset:
    ...
    LDR R4,=GPIO_PIN0
    // Banderas que sirven para que no se presiona le tecla varias veces con un rebote de tecla
    MOV R5,#0 
    MOV R6,#0
    /****************************************************************************/
main:
...
btn1:   
    ...
    BLEQ aumentar    // Rutina de aumento
    BL retraso
    POP {PC}
    .pool
    .endfunc
...
btn2:   
    ...
    BLEQ disminuir  // Rutina de disminuir
    BL retraso
    POP {PC}
    .pool
    .endfunc
...
aumentar:
    ...
    // Si llega a (periodo/2) es el maximo duty factor (Considero que esta al brillo maximo al 50% del periodo)
    CMP R2,#49
    ITE EQ
    MOVEQ R2,#1     // Setea el Duty Factor al valor inicial 1T
    ADDNE R2,#4     // Sino aumento en 4 el Factor
    STRB R2,[R12,#factor]
...
disminuir:
    ...
    // Si el factor actual es 1 si disminuyo hay desborde
    CMP R2,#1
    ITE EQ
    MOVEQ R2,#49      // Setea el Duty Factor a su valor maximo (Considero que esta al brillo maximo al 50% del periodo)
    SUBNE R2,#4       // Si no disminuyo el Factor en 2T
    STRB R2,[R12,#factor]
